...
from apps.events.models import Event, EventTag, EventSeat
...
class EventSeatAPITestCase(APITestCase):
    ...
    def test_get_all_seats_of_a_event(self):
        event_1 = Event.objects.create(name="Happy New Year",
        ...
        event_2 = Event.objects.create(name="Chrismas",
                                       user=self._user_admin,
                                       status=Event.Status.RUNNING,
                                       venue=baker.make(Venue),
                                       start_date=datetime.datetime(2022, 6, 1, 7, 30, 30, tzinfo=pytz.UTC),
        ...
        response = self._client_general.get(self.EVENT_SEATS_LIST_PATH, {"event_seat_type__event": event_1.id})
